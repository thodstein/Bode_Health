...
window.AppState = { stack:[], plan:[], wIdx:0, xp:0, charts:{liver:true, cardio:true, hemato:true} };
...
window.switchTab = function(tabId) {
    ...
    // Подсветка кнопки (ищем по onclick атрибуту для простоты)
...
window.renderStack = function() {
    ...
    AppState.stack.forEach((it, idx) => {
        const s = DB.subs.find(x=>x.id===it.sub);
...
window.calcFert = function() {
    ...
    if(el) el.innerHTML = `<span style="color:${res>50?'#03dac6':'#cf6679'}">IF: ${res}/100</span>`;
};
...
window.renderShop = function() {
    ...
    for(let k in DB.shop) {
        DB.shop[k].forEach(i => div.innerHTML += `<div class="item"><div><b>${k.toUpperCase()}</b><br>${i.p}</div><div style="color:#03dac6">${i.pr}</div></div>`);
    }
};
