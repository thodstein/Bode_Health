window.App = {
  ...
  switchTab: function(id) {
    ...
    // Находим кнопку по тексту или индексу, так как event может быть не передан
    const btns = document.querySelectorAll('.tab-btn');
    if(id==='dashboard') btns[0].classList.add('active');
    else if(id==='stack') btns[1].classList.add('active');
    else if(id==='risks') { btns[2].classList.add('active'); setTimeout(()=>this.renderChart(), 50); }
    else if(id==='support') btns[3].classList.add('active');
    else if(id==='labs') btns[4].classList.add('active');
    else if(id==='articles') btns[5].classList.add('active');
    else if(id==='shop') btns[6].classList.add('active');
  },
  loadEsters: function() {
  ...
  renderSupport: function() {
    ...
    DB.support.forEach(b => {
      div.innerHTML += '<div class="time-block"><h3>'+b.t+'</h3>'+b.items.map(i=>'<div class="item" style="margin:5px 0; padding:10px; display:block;"><b>'+i.n+'</b> '+i.d+'<br><small>'+i.m+'</small></div>').join('')+'</div>';
    });
  },
  calcFert: function() {
