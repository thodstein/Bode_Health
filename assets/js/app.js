const App = {
    ...
    loadEsters: function() {
        ...
        const estSel = document.getElementById('est-select'); estSel.innerHTML = '';
        const list = DB.esters[subId];
        if(list && list.length) {
            estSel.disabled = false;
            list.forEach(e => { let o = document.createElement('option'); o.value = e.id; o.innerText = e.name + ' (' + e.hl + ' дн.)'; estSel.appendChild(o); });
    ...
    calcFert: function() {
        ...
        const el = document.getElementById('fert-res'); if(el) el.innerHTML = '<span style="color:'+(res>50?'var(--sec)':'var(--err)')+'">IF: '+res+'/100</span>';
    },
    renderArticles: function() { const div = document.getElementById('articles-list'); if(!div) return; DB.articles.forEach(a => div.innerHTML += '<div class="item"><div><b>' + a.t + '</b><br><small>' + a.c + ' | 👁' + a.v + '</small></div></div>'); },
    renderShop: function() { const div = document.getElementById('shop-list'); if(!div) return; for(let k in DB.shop) { DB.shop[k].forEach(i => div.innerHTML += '<div class="item"><div><b>' + k.toUpperCase() + '</b><br>' + i.p + '</div><div style="color:var(--sec)">' + i.pr + '</div></div>'); } },
    renderGlossary: function() { const div = document.getElementById('glossary-list'); if(!div) return; for(let k in DB.glossary) div.innerHTML += '<div class="item"><b>' + k + '</b><br><small>' + DB.glossary[k] + '</small></div>'; },
    updateProfile: function() { const xpEl = document.getElementById('prof-xp'); const trustEl = document.getElementById('prof-trust'); if(xpEl) xpEl.innerText = this.state.xp; if(trustEl) trustEl.innerText = Math.min(100, Math.floor(this.state.xp / 10)); }
};
document.addEventListener('DOMContentLoaded', () => { console.log('DOM Ready.'); App.init(); setInterval(() => App.updateProfile(), 1000); });
